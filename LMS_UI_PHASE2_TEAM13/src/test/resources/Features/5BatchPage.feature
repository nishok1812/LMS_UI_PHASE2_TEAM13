Feature: Batch Page Functionally
  ...
  Scenario: Check if the fields exist in pop
    Given A new pop up with Batch details appears Before Admin is on dashboard page after Login admin clicks ""Batch"" from navigation bar admin clicks ""+ A New Batch"" button
  ...
  Scenario: Validate row level delete icon
    Given The delete icon on row level in data table is enabled before admin is on dashboard page after Login admin clicks "Batch" from navigation bar
    When Admin clicks the delete icon
    Then Alert appears with <yes> and <No> option

  @DeleteBatch @BatchPage_TC017
  Scenario: Validate accept alert
    Given Admin clicks the delete icon before admin is on dashboard page after Login admin clicks "Batch" from navigation bar
    When You click <yes> option
    Then Batch deleted alert pops and batch is no more available in data table

  @DeleteBatch @BatchPage_TC018
  Scenario: Validate reject alert
    Given Admin clicks the delete icon admin is on dashboard page after Login admin clicks "Batch" from navigation bar
  ...
  Scenario: Check for multi row delete
    Given Two or more checkboxes/row is selected before admin is on dashboard page after Login admin clicks ""Batch"" from navigation bar
    When Click <delete> icon below "Manage Batch" header
  ...
  Scenario Outline: Check if the fields are updated
    Given Admin clicks the edit icon before admin is on dashboard page after Login admin clicks ""Batch"" from navigation bar
